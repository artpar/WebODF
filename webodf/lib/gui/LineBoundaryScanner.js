...
gui.LineBoundaryScanner = function () {
     ...
     * return 0.
     *
     * @param {!webodfcore.SimpleClientRect} rect1
     * @param {!webodfcore.SimpleClientRect} rect2
     * @return {!number}
     */
     ...
     *
     * @param {!webodfcore.SimpleClientRect} nextRect Client rect of next step (by direction)
     * @return {!boolean}
     */
    function isLineBoundary(nextRect) {
        if (lineRect) {
            ...
            return verticalOverlapPercent(/**@type{!webodfcore.SimpleClientRect}*/(lineRect), nextRect) <= MIN_OVERLAP_THRESHOLD;
        }
        return false;
    }
    ...
    /**
     * @param {?webodfcore.SimpleClientRect} originalRect
     * @param {?webodfcore.SimpleClientRect} newRect
     * @return {?webodfcore.SimpleClientRect}
     */
    function growRect(originalRect, newRect) {
        if (originalRect && newRect) {
            return combineRects(/**@type{!webodfcore.SimpleClientRect}*/(originalRect),
                                /**@type{!webodfcore.SimpleClientRect}*/(newRect));
        }
    ...
    this.process = function(stepInfo, previousRect, nextRect) {
        ...
        var isOverLineBoundary = nextRect && isLineBoundary(/**@type{!webodfcore.SimpleClientRect}*/(nextRect));
