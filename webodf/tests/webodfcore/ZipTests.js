 ...
 */
webodfcore.ZipTests = function ZipTests(runner) {
    "use strict";
    var r = runner,
        t;

    function testNonExistingFile(callback) {
        var zip = new webodfcore.Zip("whatever", function (err) {
            t.err = err;
    ...
    function testNonZipFile(callback) {
        var path = "webodfcore/ZipTests.js";
        ...
        runtime.readFile(path, "binary", function (error, data) {
            ...
            t.zip = new webodfcore.Zip("webodfcore/ZipTests.js", function (err) {
                t.err = err;
                r.shouldBeNonNull(t, "t.err");
                callback();
            });
    ...
    function testHi(path, callback) {
        path = r.resourcePrefix() + path;
        t.zip = new webodfcore.Zip(path, function (err, zip) {
            t.err = err;
            t.zip = zip;
    ...
    function testHiUncompressed(callback) {
        testHi("webodfcore/hi-uncompressed.zip", callback);
    }

    function testHiCompressed(callback) {
        testHi("webodfcore/hi-compressed.zip", callback);
    }
...
webodfcore.ZipTests.prototype.description = function () {
    "use strict";
